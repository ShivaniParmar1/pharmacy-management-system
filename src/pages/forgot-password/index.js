...
const ForgotPassword = () => {
  ...
  const handleResetRequest = async email => {
    try {
      ...
      // Get the current front-end URL dynamically
      const frontEndUrl = window.location.origin

      const response = await axios.post(
        `${backendUrl}/api/authorization/request-password-reset`,
        { email },
        {
          headers: {
            'X-Front-End-URL': frontEndUrl
          }
        }
      )
          ...
          <Box sx={{ width: '100%', maxWidth: 400 }}>
            ...
            <form noValidate autoComplete='off' onSubmit={handleSubmit}>
              ...
              <Button fullWidth type='submit' variant='contained' sx={{ mb: 4 }} disabled={loading}>
                {!loading ? ' Send reset link' : <CircularProgress />}
              </Button>
